...
bool DataBase::setDB(DB_CONFIG* _dbConfig)
{
    ...
    if (file.empty() == false)
    {
        ...
        success = BUTIL::Util::folderCreates(file);
    }

    if (success == false) {
        int error = ::GetLastError();
        ERRORMBOX(BUTIL::Util::GetLastErrorAsString(error) + file);
        exLOGERROR("%s - %s", BUTIL::Util::GetLastErrorAsString(error), file.c_str());
        setConnected(false);
        return false;
    }
...
string DataBase::AllMoviesFSGetFilter()
{
    ...
    string folderPath = GETCM.getConfigStr(CONF_APP_CURRENTFOLDER) + "%";
...
void DataBase::setFilterSelectedValue(filterTypes _filter, string _value)
{
    ...
    BUTIL::Util::rtrim(_value);
    filters.nameSelected[_filter] = _value;
    int id=0;
    char first;
    switch (_filter)
    {
    ...
    case FILTERS_YEARMAX:
    case FILTERS_YEARMIN:
        id = atoi(_value.c_str());
        if (id == 0) id = -1;
        break;
    case FILTERS_IMDBRATINGMAX:
    case FILTERS_IMDBRATINGMIN:
        if (_value.empty() == false && _value[1] == '.')
            _value.erase(1, 1);
        id = atoi(_value.c_str());
        if (id == 0) id = -1;
        break;
