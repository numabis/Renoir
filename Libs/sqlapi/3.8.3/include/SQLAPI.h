...
typedef unsigned int (SQLAPI_CALLBACK *saLongOrLobWriter_t)(SAPieceType_t &ePieceType, void *pBuf, unsigned int nLen, void *pAddlData);

//! Long or LOB reader callback, use for field fetching
typedef void (SQLAPI_CALLBACK *saLongOrLobReader_t)(SAPieceType_t ePieceType, void *pBuf, unsigned int nLen, unsigned int nBlobSize, void *pAddlData);
...
#define GetWideCharsLength GetLength
	...
	int GetUTF8CharsLength() const;
	// assing UTF8 data
	void SetUTF8Chars(const char* szSrc, int nSrcLen = -1);
#else	// !SA_UNICODE
